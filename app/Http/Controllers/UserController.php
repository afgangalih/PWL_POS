    ...
    public function show_ajax(string $id)
{
    ...
    public function update_picture(Request $request)
    {
        // Validasi file
        $request->validate([
            'profil_picture' => 'required|image|mimes:jpeg,png,jpg|max:2048',
        ]);
        ...
        try {
            ...
            if ($userModel->profil_picture && file_exists(storage_path('app/public/' . $userModel->profil_picture))) {
                Storage::disk('public')->delete($userModel->profil_picture);
            }

            $fileName = 'profile_' . $userId . '_' . time() . '.' . $request->profil_picture->extension();
            $path = $request->profil_picture->storeAs('profiles', $fileName, 'public');
